...
for APP in $APPS; do
  ...
  build release-v5.5 "$APP" "esp32 esp32s2 esp32s3 esp32c2 esp32c3 esp32c6"
  ...
  if [ "$APP" = "examples/get-started/hello_world" ]; then
    ...
    build release-v5.3 "$APP" "esp32h2 esp32p4"
...
build latest components/esp_driver_usb_serial_jtag/test_apps/usb_serial_jtag "esp32s3 esp32c3 esp32c6 esp32h2 esp32p4"
